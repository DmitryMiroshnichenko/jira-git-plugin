...
public class GitRevisionsTabPanel extends AbstractIssueTabPanel {
    ...
    public List<IssueAction> getActions(Issue issue, User remoteUser) {
        try {
            ...
            if (logEntries == null) {
                GenericMessageAction action = new GenericMessageAction(getText("git.no.index.error.message"));
                return EasyList.build(action);
            } else if (logEntries.size() == 0) {
                GenericMessageAction action = new GenericMessageAction(getText("git.no.log.entries.message"));
                return EasyList.build(action);
			} else {
